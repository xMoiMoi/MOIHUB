...
@code_file_blpr.route('/favorites', methods=["GET"])
@login_required
def favorite_code_files():
    srp = sirope.Sirope()
    user_email = current_user.email
    code_files = list(srp.filter(CodeFile, lambda cf: cf.user_email == user_email and cf.is_favorite))
    return render_template('favorite_code_files.html', code_files=code_files, srp=srp)

@code_file_blpr.route('/like/<safe_code_id>', methods=["POST"])
@login_required
def like_code_file(safe_code_id):
    srp = sirope.Sirope()
    code_id = srp.oid_from_safe(safe_code_id)
    code_file = srp.load(code_id)
    if code_file:
        if not code_file.is_liked:
            code_file.likes += 1
            code_file.is_liked = True
        else:
            code_file.likes -= 1
            code_file.is_liked = False
        srp.save(code_file)
        flash("Code file liked/unliked.", "success")
    else:
        flash("Code file not found.", "error")
    return redirect(url_for('dashboard'))
